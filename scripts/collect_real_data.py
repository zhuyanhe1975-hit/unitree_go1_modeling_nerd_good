...
class RealMotorInterface:
    def __init__(self, cfg: dict, max_torque_nm: float):
        ...
        def _try_add_sdk_path(p: str) -> bool:
            p = os.path.abspath(p)
            if os.path.isdir(p):
                # Ensure dependent .so can be found at import time.
...
def main():
    ...
    max_speed = float(get(cfg, "real.max_speed_rad_s", required=False, default=0.0))
    ...
    try:
        while True:
            ...
            if qd_m_prev is None:
                qdd_m = 0.0
            else:
                qdd_m = (qd_m - qd_m_prev) / dt
            qd_m_prev = qd_m
            if max_speed > 0.0 and abs(qd_m) > max_speed:
                raise RuntimeError(f"qd_m={qd_m:+.3f} rad/s exceeds limit {max_speed:+.3f} rad/s")
            ...
            logs["qdd_m"].append(qdd_m)
            logs["q_out"].append(q_out)
            logs["qd_out"].append(qd_out)
            logs["qdd_out"].append(qdd_out)
        ...
        qdd_m_np = np.asarray(logs["qdd_m"], dtype=np.float64)
